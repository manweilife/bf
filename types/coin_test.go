...
func balanceObject(val uint64) SafeBfcBigInt[uint64] {
	return NewSafeBfcBigInt(val)
}

func TestCoins_PickBFCCoinsWithGas(t *testing.T) {
	// coins 1,2,3,4,5
	testCoins := Coins{
	...
	for _, tt := range tests {
		t.Run(
			tt.name, func(t *testing.T) {
				got, got1, err := tt.cs.PickBFCCoinsWithGas(tt.args.amount, tt.args.gasAmount, tt.args.pickMethod)
				if (err != nil) != tt.wantErr {
					t.Errorf("Coins.PickBFCCoinsWithGas() error = %v, wantErr %v", err, tt.wantErr)
					return
				}
				if len(got) != 0 && len(tt.want) != 0 {
					if !reflect.DeepEqual(got, tt.want) {
						t.Errorf("Coins.PickBFCCoinsWithGas() got = %v, want %v", got, tt.want)
					}
				}
				if !reflect.DeepEqual(got1, tt.want1) {
					t.Errorf("Coins.PickBFCCoinsWithGas() got1 = %v, want %v", got1, tt.want1)
				}
			},
		)
	}
...
func TestPickupCoins(t *testing.T) {
	coin := func(n uint64) Coin {
		return Coin{Balance: balanceObject(n), CoinType: BFC_COIN_TYPE}
	}
	...
	tests := []struct {
	...
	}{
		{
			name: "moreCount = 3",
			args: args{
				inputCoins: &Page[Coin, bfcObjectID]{
					Data: []Coin{
