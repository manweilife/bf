...
type Coin struct {
	...
	CoinObjectId bfc_types.ObjectID     `json:"coinObjectId"`
	Version      SafeBfcBigInt[uint64]  `json:"version"`
	Digest       bfc_types.ObjectDigest `json:"digest"`
	Balance      SafeBfcBigInt[uint64]  `json:"balance"`

	LockedUntilEpoch    *SafeBfcBigInt[uint64]      `json:"lockedUntilEpoch,omitempty"`
	PreviousTransaction bfc_types.TransactionDigest `json:"previousTransaction"`
}

func (c *Coin) Reference() *bfc_types.ObjectRef {
...
type CoinPage = Page[Coin, bfc_types.ObjectID]

func (c *Coin) IsBFC() bool {
	return c.CoinType == BFC_COIN_TYPE
}

type Balance struct {
	CoinType        string                              `json:"coinType"`
	CoinObjectCount uint64                              `json:"coinObjectCount"`
	TotalBalance    BfcBigInt                           `json:"totalBalance"`
	LockedBalance   map[SafeBfcBigInt[uint64]]BfcBigInt `json:"lockedBalance"`
}

type Supply struct {
	Value SafeBfcBigInt[uint64] `json:"value"`
}
...
func (cs *PickedCoins) CoinIds() []bfc_types.ObjectID {
	coinIds := make([]bfc_types.ObjectID, len(cs.Coins))
...
func (cs *PickedCoins) CoinRefs() []*bfc_types.ObjectRef {
	coinRefs := make([]*bfc_types.ObjectRef, len(cs.Coins))
	for idx, coin := range cs.Coins {
...
// PickBFCCoinsWithGas pick coins, which sum >= amount, and pick a gas coin >= gasAmount which not in coins
...
func (cs Coins) PickBFCCoinsWithGas(amount *big.Int, gasAmount uint64, pickMethod int) (Coins, *Coin, error) {
	if gasAmount == 0 {
